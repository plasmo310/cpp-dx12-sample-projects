...
void DXApplication::OnUpdate()
{
	// 回転させてみる
	angle_ += 1.0f;
	worldMatrix_ = worldMatrix_ = DirectX::XMMatrixRotationY(angle_ * (DirectX::XM_PI / 180.0f)) * DirectX::XMMatrixScaling(scale_, scale_, 1.0f);
	*mapMatrix_ = worldMatrix_ * viewMatrix_ * projMatrix_;
}
...
void DXApplication::OnRender()
{
	...
	// コマンドリストの生成
	{
		...
		commandList_->ClearRenderTargetView(rtvHandle, clearColor, 0, nullptr); // 画面クリア
		...
		commandList_->DrawIndexedInstanced(36, 1, 0, 0, 0);                        // 描画
